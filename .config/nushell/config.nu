  ...
  menus: [
      ...
      # --------- My custom menus
      {
        ...
        source: { |buffer, position|
            ls ~/.tmuxp/ 
            | each { |it| {value: $it.name description: (open $it.name| get session_name)} }
        }
      }
      # --------- End my custom menus
  ...
  keybindings: [
    ...
    }
    {
      ...
      event: {
        ...
        cmd: "pass -c (fd --base-directory ~/.password-store/ .*.gpg | sed -r 's,(.*)\\.gpg,\\1,' | fzf)"
      }
    }
    {
      name: launch_tmuxp_session
      modifier: control
      keycode: char_s
      mode: [emacs, vi_normal, vi_insert]
      # event: { send: menu name: tmuxp_menu }
      event: {
        send: executehostcommand
        cmd: "tmuxp load -y (ls ~/.tmuxp| each { |it| $it.name} | str join (char nl) | fzf)"
      }
    }
  ]
}
